...
const SOURCE_POLICIES: Record<string, StrainSourcePolicy> = {
  leafly: {
    ...
    summary: "Primärquelle für breite Abdeckung und schnelle Bild-Recherche.",
  },
  allbud: {
    key: "allbud",
    label: "AllBud",
  ...
  askgrowers: {
    ...
    summary: "Nur nach manueller Prüfung verwenden; Stock- oder Similar-Image-Risiko.",
  },
  straindb: {
    key: "straindb",
    ...
    tier: "review",
    requiresSourceNotes: true,
    summary: "Breite Datenquelle; Import vor Veröffentlichung in der Admin-Queue prüfen.",
  },
  "manual-curation": {
    key: "manual-curation",
    ...
    tier: "primary",
    requiresSourceNotes: false,
    summary: "Manuell geprüft und für GreenLog kuratiert.",
  },
  unknown: {
    key: "unknown",
    label: "Unbekannt",
    tier: "review",
    requiresSourceNotes: true,
    summary: "Quelle unklar; vor Publikation dokumentieren und manuell prüfen.",
  },
};
...
export function buildDefaultSourceNotes(value?: string | null): string | null {
  ...
  switch (policy.key) {
    case "leafly":
      return "Leafly priorisieren, Bild vor Veröffentlichung prüfen und anschließend selbst hosten.";
    case "allbud":
      return "AllBud nur als Fallback nutzen, wenn Leafly kein brauchbares Bild liefert; Bild vor Veröffentlichung selbst hosten.";
    case "askgrowers":
      return "AskGrowers nur nach manueller Prüfung verwenden; stock/similar Hinweise ausschließen und Bild vor Veröffentlichung selbst hosten.";
    case "straindb":
      return "Strain Database Import prüfen, Quellenhinweise dokumentieren und Bild vor Veröffentlichung selbst hosten.";
    case "manual-curation":
      return "Manuell kuratiert und für GreenLog freigegeben; Bild vor Veröffentlichung selbst hosten.";
    default:
      return null;
  }
}
...
export function detectSourceWarnings(input: {
  ...
  if (policy.requiresSourceNotes && !input.sourceNotes?.trim()) {
    warnings.push("Quelle braucht dokumentierte manuelle Prüfung.");
  }
