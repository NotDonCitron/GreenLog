      ...
      >
        {strains.length > 0 ? (
                ...
                >
                  <Card className={`absolute inset-0 backface-hidden overflow-hidden border-2 rounded-3xl bg-[#1a191b] shadow-2xl transition-all duration-300 ${isTop ? 'border-[#00F5FF] ring-4 ring-[#00F5FF]/20 shadow-[#00F5FF]/10' : 'border-white/10'}`}>
                    ...
                    <div className="h-2/3 relative">
                      <img src={strain.image_url || 'https://images.unsplash.com/photo-1536859355448-76f926813d1d?auto=format&fit=crop&q=80&w=800'} alt={strain.name} className="w-full h-full object-cover" />
                    ...
                    <div className="p-5 flex flex-col h-1/3 justify-between">
                      <div>
                        <div className="flex justify-between items-start mb-1">
                          ...
                          <span className="text-[10px] text-white/40 font-mono">#{strain.id.slice(0,4)}</span>
                        </div>
                        <h2 className="text-2xl font-black italic tracking-tighter uppercase leading-none mb-1">{strain.name}</h2>
                        <p className="text-xs text-white/50 font-medium uppercase tracking-widest">VERIFIED STRAIN</p>
                      </div>
          ...
          <div className="text-center">
            <p className="text-white/40 mb-4">Deine Datenbank ist noch leer.</p>
            <p className="text-xs text-[#00F5FF]">Füge Strains im Supabase Dashboard hinzu!</p>
          </div>
        )}
