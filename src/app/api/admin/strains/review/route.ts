...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase
    .from("strains")
    .select(
      "id, name, slug, type, description, thc_min, thc_max, cbd_min, cbd_max, avg_thc, avg_cbd, terpenes, flavors, effects, image_url, canonical_image_path, primary_source, publication_status, quality_score"
    )
