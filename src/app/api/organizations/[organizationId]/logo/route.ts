...
export async function POST(
  ...
  const upload = await uploadToMinio("org-logos", key, buffer, image.type, { upsert: false });
  ...
  if (updateError) return jsonError("Failed to update organization logo", 500, updateError.code, updateError.message);

  return jsonSuccess({ logo_url: upload.publicUrl, path: upload.path });
}
