...
const effectPattern = new RegExp(`(?:${EFFECT_KEYWORDS.map(escapeRegex).join('|')})`, 'gi');
...
export async function POST(req: NextRequest) {
    try {
        ...
        const allMatchedEffects = new Set([...lowerHtml.matchAll(effectPattern)].map(m => m[0].toLowerCase()));

        EFFECT_KEYWORDS.forEach((keyword) => {
            if (allMatchedEffects.has(keyword)) {
