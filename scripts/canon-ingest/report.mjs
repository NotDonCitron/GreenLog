...
export class IngestReport {
  constructor() {
    ...
    this.totalUpdated = 0;
    this.totalPartialUpdates = 0;
    this.totalEnrichments = 0;
    this.totalErrors = 0;
    this.errors = [];
    this.mergeStats = {
      groupsProcessed: 0,
  ...
  recordPartialUpdate() { this.totalPartialUpdates++; }
  recordEnrichment() { this.totalEnrichments++; }
  ...
  recordMergeStats(stats) {
    this.mergeStats.groupsProcessed += stats.groupsProcessed || 0;
    this.mergeStats.conflictsDetected += stats.conflictsDetected || 0;
    this.mergeStats.conflictsResolved += stats.conflictsResolved || 0;
    this.mergeStats.fieldsMerged += stats.fieldsMerged || 0;
  }

  recordImageResult(source, success) {
    this.imageStats.attempted++;
    if (success) {
      this.imageStats.downloaded++;
      this.imageStats.uploaded++;
    } else {
      this.imageStats.failed++;
    }
    this.imageStats.bySource[source] = this.imageStats.bySource[source] || { success: 0, failed: 0 };
    if (success) this.imageStats.bySource[source].success++;
    else this.imageStats.bySource[source].failed++;
  }
  ...
  printReport() {
    ...
    console.log(`  🔧 Partial:       ${this.totalPartialUpdates}`);
    console.log(`  ✨ Enriched:      ${this.totalEnrichments}`);
    console.log(`  ⚠️  DB Errors:     ${this.totalErrors}`);

    console.log('\n' + '─'.repeat(60));
    console.log('  Merge Stats:');
    console.log(`     Groups: ${this.mergeStats.groupsProcessed} | Conflicts: ${this.mergeStats.conflictsDetected} | Fields merged: ${this.mergeStats.fieldsMerged}`);

    console.log('\n' + '─'.repeat(60));
    console.log('  Image Pipeline:');
    console.log(`     Attempted: ${this.imageStats.attempted} | Downloaded: ${this.imageStats.downloaded} | Failed: ${this.imageStats.failed}`);
    for (const [src, stats] of Object.entries(this.imageStats.bySource)) {
      console.log(`     ${src}: ✓${stats.success} ✗${stats.failed}`);
    }
  ...
  exportJSON() {
    ...
    const report = {
      ...
      summary: {
        ...
        total_partial_updates: this.totalPartialUpdates,
        total_enrichments: this.totalEnrichments,
        total_errors: this.totalErrors,
        success_rate: this.successRate + '%',
      },
      merge_stats: this.mergeStats,
      image_stats: this.imageStats,
