...
(async () => {
  ...
  // Find and click button via evaluate (bypasses visibility checks)
  const result = await page.evaluate(() => {
    ...
    // Set values using native setters
    const nativeSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
    nativeSetter.call(emailInput, 'greenlog-test-2026@example.com');
    emailInput.dispatchEvent(new Event('input', { bubbles: true }));

    // Click the button directly via JS
    btn.click();
  ...
  } else if (page.url() !== 'http://localhost:3000/sign-in') {
