...
ws.on('message', (data) => {
  ...
  if (step === 1) {
    setTimeout(() => {
      const expr = "const sk=document.documentElement.outerHTML.match(/sk_live_[A-Za-z0-9_-]+/g);JSON.stringify({sk:sk})";
      ws.send(JSON.stringify({ id: 2, method: 'Runtime.evaluate', params: { expression: expr } }));
    }, 12000);
  } else if (step === 2) {
    console.log('SK matches:', msg.result.result.value);
    ws.close();
    process.exit(0);
  }
});
