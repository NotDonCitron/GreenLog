...
CREATE POLICY "Organizations are viewable by everyone"
  ...
  USING (status = 'active' OR (auth.uid())::text = created_by);
...
-- Helper function to check if user is active member of org (bypasses RLS recursion)
...
CREATE POLICY "Members can view org members"
  ON organization_members FOR SELECT
  USING (is_active_org_member(auth.uid(), organization_id));
...
CREATE POLICY "Admins can create invites"
  ...
  WITH CHECK (is_active_org_member(auth.uid(), organization_id));
...
CREATE POLICY "Users see own requests"
  ...
  USING ((auth.uid())::text = requester_id OR (auth.uid())::text = target_id);
